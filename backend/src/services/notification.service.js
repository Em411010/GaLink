 ...
 */
export async function createNotification({ recipient, sender, type, post = null, reel = null, message = "" }) {
  // Don't notify yourself
  if (recipient.toString() === sender.toString()) return null;

  const notification = await Notification.create({ recipient, sender, type, post, reel, message });
